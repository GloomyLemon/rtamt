...
// Explicit P behavior
TEST(BoundedTemporalUnaryExplicitTests, PreviousNodeCorrectness) {
    ...
    for (int t = 0; t < static_cast<int>(phi.size()); ++t) {
        ...
        if constexpr (std::is_same_v<typename TestFixture::Op, TimedHistoricallyNode>) {
            ...
            for (int tau = t - end; tau <= t - begin; ++tau) {
                ...
                double right = Hpsi[tau];

                double left = +std::numeric_limits<double>::infinity();
                for (int k = tau + 1; k <= t; ++k)
                    left = std::min(left, Hphi[k]);
